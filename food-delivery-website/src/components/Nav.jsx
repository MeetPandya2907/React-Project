...
function Nav() {
  ...
  return (
    <div className='w-full h-[100px]  flex justify-between items-center px-5 md:px-8'>
      ...
      <form className='w-[45%] h-[60px] bg-white flex items-center px-5 gap-5 rounded-md shadow-md md:w-[70%]' onSubmit={(e)=>{
        e.preventDefault()
      }}>
        <IoSearch className='text-green-500 w-[20px] h-[20px] '/>
        <input type="text" placeholder='Search Items...' onChange={(e)=>{
          setInput(e.target.value)
        }} value={input} className='w-[100%] outline-none text-[16px] md:text-[20px]' />
       </form>
